<template>
  <div >
    <vHeader :currentHeader="4" :searchMessage="searchData.searchTitle" :sortType="sortType" ref="navHeader"></vHeader>
  ...
  padding-bottom: 50px;">
                ...
                <section id="view" clock>
                  <div class="demo" style="margin-top: 4%">
                    <div class="demo-warp" style="margin-top: 4%">
                      <div class="demo-flex" style="width: 90%;margin-left: 5%;" v-for="(v,k) in filterList" :key="k">
                      ...
                      <div class="demo-flex" style="width: 90%;margin-left: 5%;" v-for="(vt,j) in themeList" :key="j">
                        <span class="demo-title">{{vt.title}}</span>
                        <div class="demo-content">
                          <div class="demo-tab" :class="isShow2 ? 'demo-hide' : ''">
                            <span v-for="(valt, keyt) in vt.childer" :key="keyt" :class="{'demo-active': valt.active}" @click="tabThemeClick(valt,keyt,j)">{{valt.name}}</span>
                          </div>
                        </div>
                        <div class="demo-more" @click="isShow2 = !isShow2" v-if="vt.childer.length>11">更多</div>
                      </div>
    ...
    margin-left: 5%;" v-for="(va,i) in sortList" :key="va.id">
                        ...
                        <div class="demo-content">
                          <div class="demo-tab" >
                            <span v-for="(value, index) in va.childer" :key="index" :class="{'demo-active': value.active}" @click="tabClick(value,index,i)">{{value.name}}</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <ul id="show">
                      ...
                      <div role="tabpanel" class="tab-pane active" id="popularity" >
                        <div class="row" style="margin-top: 2%" v-for="(va,index) in hotelList">
                          <div class="col-md-3"  v-for="hotel in va">
                            <router-link :to="{path:'lines',query:{lineId:hotel.lineId}}" style="width: 90%;margin-left: 5%;height: 250px">
                              <img :src="hotel.linePhoto" style="height: 148px" alt="">
                              <div class="has_border" style="height: 94px;">
                                <h3>{{hotel.lineName}}</h3>
                                <div class="price" style="text-align: right;"><strong>网付价<em>￥</em><strong>{{hotel.linePrice+hotel.roomPrice}}</strong><em>起</em></strong></div>
                              </div>
                            </router-link>
                          </div>
                ...
                <div v-show="searchStatus2">
                  <!--                    <li>-->
                  <!--                        <div class="img"><img src="/static/images/04-search_03.jpg" alt=""></div>-->
                  <!--                        <div class="text1">-->
...
width:15%;">
              ...
              <ul style="margin-top: 8%">
                <li v-for="(line,hid) in hotList" :key="hid">
                  <div class="left"><img style="width: 100%" :src="line.scenicImg" alt=""></div>
                  <div class="right">
                    <router-link :to="{path:'scenic',query:{scenicId:line.scenicId}}" ><p>{{line.scenicName}}</p></router-link>
                    <p> <span class="price" style="text-align: right"><strong>点击查看</strong></span>
                    </p>
                  </div>
                </li>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      tabThemeClick(data, key, k) {
        // 添加 active ==> true 显示 `active样式`
        this.themeList[k].childer.map(item => {
          item.active = false
        })
        this.themeList[k].childer[key].active = true

        // 选中的数据
        let newArray = []
        this.themeList.map(data => {
          data.childer.map(item => {
            if (item.active === true) {
              this.searchData.themeId=item.value;
              this.searchHotel();
            }
          })
        })
      ...
      initHot() {
        ...
        console.log(this.searchData);
        this.$axios.post("scenic/scenic_list_hot")
          .then(result=>{
            var data = result.data;
            this.hotList=[];
            this.hotList=data.scenicList;
      ...
      //  alert(index)
        ...
        this.$axios.post("line_group/line_list",qs.stringify(this.searchData),config)
          .then(result=>{
            var data = result.data;
            this.hotelList=[];
            this.hotelList=data.lineGroupList;
            this.all =data.pageCount;//总页数
            this.totalCount = data.total;
            this.searchData.cityId=data.cityId;
            this.searchData.themeId=data.themeId;
...
//根据点击页数请求数据
      ...
      init(){
        ...
        this.$axios.get("/scenic/nav_scenic_init")
          .then(successReqponse => {
            ...
            this.child2=successReqponse.data.sortChild2;
            this.themeList=[{
              "title": "主题：",
              "childer":this.child2
            }
