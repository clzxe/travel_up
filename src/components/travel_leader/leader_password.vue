    ...
    methods: {
      ...
      init(){
        this.$axios.get("leader/getLeader")
          .then(response => {
            // alert(data.flag);
            this.user=response.data.leader;
          })
          .catch(failResponse => {
            alert("响应异常")
          })
      ...
      sub(){
        if(this.checkPassword()&&this.checkPasswordAgain()&&this.checkOldPassword()){
          var qs = require('querystring')

          let config = {
            header: {'Content-type': 'application/x-www-form-urlencoded'}
          };
          this.userMessage.userId=this.user.leaderId;
          this.userMessage.password=this.newPassword;
          this.$axios.post("/leader/update_password", qs.stringify(this.userMessage), config)
            .then(successReqponse => {
              if(successReqponse.data==="OK"){
                alert("修改成功")
                var url=this.$router.resolve('LeaderLogin');
