<template>
<div >
  <VNav :currentNav="8" :currentStr="1"></VNav>
  <section class="rt_wrap content mCustomScrollbar">
    <div class="rt_content">
      <div class="page_title">
        <h2 class="fl">旅行团列表</h2>
      </div>
      <section class="mtb">
      ...
      <table class="table table-bordered">
        ...
        <tbody  v-for="(group,index) in groupList">
       <tr>
         <th><input type="checkbox" name="id" v-model="delId" :value=group.groupId></th>
         <td style="padding-left: 20px;max-width: 200px;">{{group.groupName}}</td>
         <td style="padding-left: 20px;">{{group.groupDes}}</td>
         <td style="text-align: center;">{{group.groupCeatetime}}</td>
        <td class="center">
          <a  title="修改" data-toggle="modal" data-target="#myModal3" class="iconfont travel-iconedit"  style="color: #c12e2a;font-size: 25px" @click="update(group)" ></a>
          <a  title="删除"  class="iconfont travel-iconshanchu"  style="color: #00a0e9;font-size: 25px"  @click="del(group.groupId)"></a>
          <a  title="查看" data-toggle="modal" data-target="#myModal3"  class="iconfont travel-iconchakan"  style="color: #ff79c6;font-size: 29px"  @click="getView(group)"></a>
        </td>
       </tr>
        </tbody>
      ...
      <div class="modal-dialog" style="width:900px;height: auto">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form class="form-horizontal" role="form" onsubmit="return false;" id="form1" v-show="themeStatus===4">
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <el-input type="text"  style="width: 60%" v-model="groupUpdate.groupName"
                            name="scenicName" placeholder="主题名称" required="required"></el-input>
                </div>
                <label class="col-sm-2 control-label">旅行团简介</label>
                <div class="col-sm-10">
                  <el-input type="text"  style="width: 60%" v-model="groupUpdate.groupDes"
                            name="scenicName" placeholder="旅行团简介" required="required"></el-input>
                </div>
              </div>
            </form>
              ...
              <ul class="cus-table">
                <li style="margin-top: 2%">
                  <label style="width: 98px">旅行团名称：</label>
                    ...
                    <el-input style="font-weight: 100;width: 60%;" readonly="readonly" :value="groupView.groupName"></el-input>

                </li>
                <li style="margin-top: 2%">
                  <label style="width: 98px">旅行团名称：</label>

                    <el-input style="font-weight: 100;width: 60%;" readonly="readonly" :value="groupView.groupDes"></el-input>
            ...
            <form  method="post" name="travel-cus" class="cusForm" v-show="themeStatus===2">
              ...
              <ul class="cus-table">
                <li style="margin-top: 2%">
                  <label style="width: 98px;margin-left: 10%">旅行团名称：</label>

                  <el-input style="font-weight: 100;width: 60%;"  v-model="groupMessage.groupName"></el-input>

                </li>
                <li style="margin-top: 2%">
                  <label style="width: 98px;margin-left: 10%">旅行团简介：</label>

                  <el-input style="font-weight: 100;width: 60%;"  v-model="groupMessage.groupDes"></el-input>
          ...
          <div class="modal-footer">
            ...
            <el-button  class="btn btn-default" data-dismiss="modal" @click="themeStatus=1" v-show="themeStatus===3">返回</el-button>
...
//			alert(editor.txt.html());
          ...
          this.$axios.post("/group/update", qs.stringify(this.groupUpdate), config)
...
//			alert(editor.txt.html());
          ...
          this.$axios.post("/group/save", qs.stringify(this.groupMessage), config)
            .then(successReqponse => {
              alert("保存成功");
              this.$router.go(0)
            })
        ...
        dataListFn: function(index){
          ...
          this.$axios.post("group/list",qs.stringify(this.searchDate),config)
            .then(result=>{
              var data = result.data;
              this.groupList=[];
              this.groupList=data.classList;
...
//根据点击页数请求数据
        ...
        del(groupId) {
          ...
          if(confirm("确实要删除吗？")){
            ...
            this.$axios.post("group/del?groupId="+groupId,config)
