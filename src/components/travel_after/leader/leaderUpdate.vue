      ...
      <div class="rt_content">
        ...
        <form class="form-horizontal" role="form" onsubmit="return false;" id="form1">
          <div class="form-group">
            ...
            <div class="col-sm-10" >
              <el-input type="text"  v-model="groupName" v-on:input="groupName1"></el-input>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" class="form-control" v-model="leader.leaderNumber" id="leaderNumber" name="leaderName" placeholder="用户名" required="required" @blur="checkUsername">
            </div>
          </div>
          <div class="form-group">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getThemes:function(){
        this.leaderId=this.$route.query.leaderId;
        var qs = require('querystring')
        let config = {
          header: {'Content-type': 'application/x-www-form-urlencoded'}
        };
        this.$axios.get("/leader/getleader?leaderId="+this.leaderId)
          .then(successReqponse => {
            this.leader=successReqponse.data.leaders;
            this.groupList=successReqponse.data.groupList;
           if(this.leader.leaderSex===1){
             this.leaderSex='女';
           }else {
             this.leaderSex='男';
           }
           this.groupName=this.leader.groupName
          })
          .catch(failRespose => {
            console.log("页面加载错误！正在重新载入...");
            this.$router.replace("/leaderUpdate?leaderId="+this.leaderId);
          })
      ...
      sub() {
        ...
        this.$axios.post("/leader/update", qs.stringify(this.leader), config)
          .then(successReqponse => {
