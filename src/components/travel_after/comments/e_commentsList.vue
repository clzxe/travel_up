  ...
  <div >
    ...
    <section class="rt_wrap content mCustomScrollbar">
      <div class="rt_content">
        ...
        <div>
          <el-button  @click="gets_comment(0)"  >所有订单</el-button>
          <el-button  @click="gets_comment(1)" >路线订单</el-button>
          <el-button   @click="gets_comment(2)" >酒店订单</el-button>
        </div>

        <table class="table table-be_commented">
          <thead>
          <tr>
            <th>全选</th>
            <th>订单编号</th>
          ...
          <tbody id="showtable" v-for="(e_comment,index) in e_commentList">
          <tr>
            <th><input type="checkbox" name="id" value=item.title></th>
            <td style="text-align: center;">{{e_comment.orderId}}</td>
            <td style="text-align: center;">{{e_comment.orderName}}</td>
            <td style="text-align: center;">{{e_comment.evaluateContent}}</td>
            <td style="padding-left: 20px;">{{e_comment.userName}}</td>
            <td style="padding-left: 20px;">{{e_comment.commentContent}}</td>
            <td style="text-align: center;">{{e_comment.createTime}}</td>

            <td class="center">
              <a  title="删除" class="iconfont travel-iconshanchu"  style="color: #00a0e9;font-size: 25px"  @click="del(e_comment.e_commentId)"></a>
              <a  title="查看" data-toggle="modal"
                  data-target="#seeScenic" class="iconfont travel-iconchakan"  style="color: #ff79c6;font-size: 29px"  @click="getReply(e_comment.e_commentId)"></a>
            </td>
          </tr>
          </tbody>
          ...
          <div class="modal-dialog" style="width: 658px;height: auto">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <h3 class="modal-title" id="seeScenicLabel">查看回复</h3>
              </div>
  ...
  export default {
    ...
    methods:{
      ...
      dataListFn: function(index){
        ...
        this.$axios.post("e_comment/list",qs.stringify(this.searchDate),config)
          .then(result=>{
            var data = result.data;
            this.e_commentList=[];
            this.e_commentList=data.e_commentList;
...
//根据点击页数请求数据
      ...
      del(commentId) {
        ...
        if(confirm("确实要删除吗？")){
          ...
          this.$axios.post("e_comment/del?commentId="+commentId,config)
